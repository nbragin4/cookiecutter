...
def test_clone_should_raise_if_vcs_not_installed(mocker, clone_dir):
    ...
    is installed."""
    mocker.patch('scaffoldrom.vcs.is_vcs_installed', autospec=True, return_value=False)

    repo_url = 'https://github.com/pytest-dev/scaffoldrom-pytest-plugin.git'
...
def test_clone_should_rstrip_trailing_slash_in_repo_url(mocker, clone_dir):
    ...
    present."""
    mocker.patch('scaffoldrom.vcs.is_vcs_installed', autospec=True, return_value=True)

    mock_subprocess = mocker.patch(
        'scaffoldrom.vcs.subprocess.check_output',
        autospec=True,
    )
...
def test_clone_should_abort_if_user_does_not_want_to_reclone(mocker, clone_dir):
    """In `clone()`, if user doesn't want to reclone, Scaffoldrom should exit \
    without cloning anything."""
    mocker.patch('scaffoldrom.vcs.is_vcs_installed', autospec=True, return_value=True)
    mocker.patch(
        'scaffoldrom.vcs.prompt_and_delete', side_effect=SystemExit, autospec=True
    )
    mock_subprocess = mocker.patch(
        'scaffoldrom.vcs.subprocess.check_output',
        autospec=True,
    ...
    repo_dir = clone_dir.joinpath('scaffoldrom-pytest-plugin')
    repo_dir.mkdir()
...
def test_clone_should_silent_exit_if_ok_to_reuse(mocker, tmpdir):
    ...
    mocker.patch('scaffoldrom.vcs.is_vcs_installed', autospec=True, return_value=True)
    mocker.patch(
        'scaffoldrom.vcs.prompt_and_delete', return_value=False, autospec=True
    )
    mock_subprocess = mocker.patch(
        'scaffoldrom.vcs.subprocess.check_output',
        autospec=True,
    ...
    # Create repo_dir to trigger prompt_and_delete
    clone_to_dir.mkdir('scaffoldrom-pytest-plugin')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('git', 'git@gitlab.com:scaffoldrom/scaffoldrom.git', 'scaffoldrom'),
        ('git', 'git@github.com:scaffoldrom/scaffoldrom.git', 'scaffoldrom'),
    ],
)
...
def test_clone_handles_branch_typo(mocker, clone_dir, error_message):
    ...
    repository_url = 'https://github.com/pytest-dev/scaffoldrom-pytest-plugin'
...
def test_clone_unknown_subprocess_error(mocker, clone_dir):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        vcs.clone(
            'https://github.com/pytest-dev/scaffoldrom-pytest-plugin',
            clone_to_dir=str(clone_dir),
            no_input=True,
        )
