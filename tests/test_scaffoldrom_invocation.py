"""
test_scaffoldrom_invocation.

Tests to make sure that scaffoldrom can be called from the cli without
...
def test_should_invoke_main(monkeypatch, project_dir):
    ...
    exit_code = subprocess.check_call(
        [sys.executable, '-m', 'scaffoldrom.cli', 'tests/fake-repo-tmpl', '--no-input']
    )
    assert exit_code == 0
