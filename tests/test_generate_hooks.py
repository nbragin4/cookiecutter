...
from scaffoldrom import generate, utils
from scaffoldrom.exceptions import FailedHookException
...
def test_ignore_hooks_dirs():
    ...
    generate.generate_files(
        context={'scaffoldrom': {'pyhooks': 'pyhooks'}},
...
def test_run_python_hooks_cwd():
    ...
    """
    generate.generate_files(
        context={'scaffoldrom': {'pyhooks': 'pyhooks'}}, repo_dir='tests/test-pyhooks/'
    )
...
def test_empty_hooks():
    ...
    with pytest.raises(FailedHookException) as excinfo:
        generate.generate_files(
            context={'scaffoldrom': {'shellhooks': 'shellhooks'}},
...
def test_oserror_hooks(mocker):
    """Verify script error passed correctly to scaffoldrom error.
...
def test_run_failing_hook_removes_output_directory():
    ...
    template = os.path.join(repo_path, 'input{{scaffoldrom.hooks}}')
    os.mkdir(repo_path)
    os.mkdir(hook_dir)
    os.mkdir(template)
    ...
    with pytest.raises(FailedHookException) as excinfo:
        generate.generate_files(
            context={'scaffoldrom': {'hooks': 'hooks'}},
