"""Tests for `scaffoldrom.utils` module."""
import stat
import sys
...
def test_prompt_should_ask_and_rm_repo_dir(mocker, tmp_path):
    ...
    mock_read_user = mocker.patch(
        'scaffoldrom.utils.read_user_yes_no', return_value=True
    )
...
def test_prompt_should_ask_and_exit_on_user_no_answer(mocker, tmp_path):
    ...
    repo, scaffoldrom should exit."""
    mock_read_user = mocker.patch(
        'scaffoldrom.utils.read_user_yes_no',
        return_value=False,
    )
...
def test_prompt_should_ask_and_rm_repo_file(mocker, tmp_path):
    ...
    mock_read_user = mocker.patch(
        'scaffoldrom.utils.read_user_yes_no', return_value=True, autospec=True
    )
...
def test_prompt_should_ask_and_keep_repo_on_no_reuse(mocker, tmp_path):
    ...
    mock_read_user = mocker.patch(
        'scaffoldrom.utils.read_user_yes_no', return_value=False, autospec=True
    )
    repo_dir = Path(tmp_path, 'repo')
    repo_dir.mkdir()
...
def test_prompt_should_ask_and_keep_repo_on_reuse(mocker, tmp_path):
    ...
    mock_read_user = mocker.patch(
        'scaffoldrom.utils.read_user_yes_no', side_effect=answer, autospec=True
    )
    repo_dir = Path(tmp_path, 'repo')
    repo_dir.mkdir()
...
def test_prompt_should_not_ask_if_no_input_and_rm_repo_dir(mocker, tmp_path):
    ...
    """
    mock_read_user = mocker.patch(
        'scaffoldrom.prompt.read_user_yes_no', return_value=True, autospec=True
    )
    repo_dir = Path(tmp_path, 'repo')
    repo_dir.mkdir()
