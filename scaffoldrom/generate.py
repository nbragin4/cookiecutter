...
def is_copy_only_path(path, context):
    ...
    :param context: scaffoldrom context.
    """
    try:
        for dont_render in context['scaffoldrom']['_copy_without_render']:
    ...
    for variable, overwrite in overwrite_context.items():
        ...
        if isinstance(context_value, list) and isinstance(overwrite, list):
            ...
            if overwrite in context_value:
                ...
                # see ``scaffoldrom.prompt.prompt_choice_for_config``
...
def generate_file(project_dir, infile, context, env, skip_if_file_exists=False):
    ...
    :param infile: Input file to generate the file from. Relative to the root
        template dir.
    :param context: Dict for populating the scaffoldrom's variables.
    ...
    if context['scaffoldrom'].get('_new_lines', False):
        # Use `_new_lines` from context, if configured.
        newline = context['scaffoldrom']['_new_lines']
...
def _run_hook_from_repo_dir(
    ...
    :param context: Scaffoldrom project context.
...
def generate_files(
    ...
    envvars = context.get('scaffoldrom', {}).get('_jinja2_env_vars', {})
