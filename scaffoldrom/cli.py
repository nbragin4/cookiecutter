...
from scaffoldrom import __version__
...
def version_msg():
    """Return the Scaffoldrom version, location and Python powering it."""
    python_version = sys.version
    location = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return f"Scaffoldrom {__version__} from {location} (Python {python_version})"
...
def list_installed_templates(default_config, passed_config_file):
    """List installed (locally cloned) templates. Use scaffoldrom --list-installed."""
    config = get_user_config(passed_config_file, default_config)
    scaffoldrom_folder = config.get('scaffoldroms_dir')
    if not os.path.exists(scaffoldrom_folder):
        click.echo(
            f"Error: Cannot list installed templates. "
            f"Folder does not exist: {scaffoldrom_folder}"
        )
        sys.exit(-1)
...
def main(
    ...
    """Create a project from a Scaffoldrom project template (TEMPLATE).

    Scaffoldrom is free and open source software, developed and managed by
    volunteers. If you would like to help out or fund the project, please get
    in touch at https://github.com/nbragin4/scaffoldrom.
    """
    ...
    try:
        scaffoldrom(
            ...
            password=os.environ.get('SCAFFOLDROM_REPO_PASSWORD'),
            directory=directory,
