...
from scaffoldrom.exceptions import UndefinedVariableInTemplate
...
def render_variable(env, raw, scaffoldrom_dict):
    ...
    Cookie", the repo_name could be be rendered with:
        ...
        `{{ scaffoldrom.project_name.replace(" ", "_") }}`.

    This is then presented to the user as the default.

    :param Environment env: A Jinja2 Environment object.
    :param raw: The next value to be prompted for by the user.
    :param dict scaffoldrom_dict: The current context as it's gradually
    ...
    if raw is None or isinstance(raw, bool):
        ...
        return {
            render_variable(env, k, scaffoldrom_dict): render_variable(
                env, v, scaffoldrom_dict
            )
            for k, v in raw.items()
        }
    elif isinstance(raw, list):
        return [render_variable(env, v, scaffoldrom_dict) for v in raw]
    elif not isinstance(raw, str):
        raw = str(raw)

    template = env.from_string(raw)

    return template.render(scaffoldrom=scaffoldrom_dict)


def prompt_choice_for_config(
    scaffoldrom_dict, env, key, options, no_input, prompts=None, prefix=""
):
    """Prompt user with a set of options to choose from.

    :param no_input: Do not prompt for user input and return the first available option.
    """
    rendered_options = [render_variable(env, raw, scaffoldrom_dict) for raw in options]
    if no_input:
...
def prompt_for_config(context, no_input=False, global_context=OrderedDict({'scaffoldrom':{}})):
    ...
    if '__prompts__' in context['scaffoldrom'].keys():
        ...
        del context['scaffoldrom']['__prompts__']
    ...
    count = 0
    all_prompts = context['scaffoldrom'].items()
